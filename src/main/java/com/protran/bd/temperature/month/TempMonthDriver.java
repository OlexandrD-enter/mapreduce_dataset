...
public class TempMonthDriver extends Configured implements Tool
{
    public int run(String[] arg0) throws Exception
    {
        try
        {
            ...
            job.setMapperClass(TempMonthMapper.class);
            job.setMapOutputKeyClass(Text.class);
            job.setMapOutputValueClass(FloatWritable.class);

            job.setReducerClass(TempMonthReducer.class);
           ...
           /* if (fs.exists(new Path(arg0[1] + " month " + before + " - " + after)))
            ...
            FileOutputFormat.setOutputPath(job, new Path(arg0[1] + "_temp_month_" + before + "_" + after));
    ...
    {
        ToolRunner.run(new TempMonthDriver(), args);
    }
}
