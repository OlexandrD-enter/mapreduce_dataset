...
public class TempWeekReducer extends Reducer<Text, FloatWritable, Text, NullWritable> {
    ...
    protected void reduce(Text key, Iterable<FloatWritable> floatTypes, Context context)
        ...
        if (hm.size() == 7) {
            ...
            context.write(new Text("Week:" + " (" + first.get().getKey() + "-" + lastEntry.getKey() +")" +"; avg_temp: " + res), nullWritable);
            hm.clear();
        }
    }
}
